...
test("surface files inside scope become read-write", () => {
  ...
  assert.deepEqual(
    resolved.resolved.files.filter((rule) => rule.permission === "read-write"),
    [
      {
...
test("surface files outside scope produce findings instead of write authority", () => {
  ...
  assert.equal(
    resolved.resolved.files.some(
      (rule) => rule.pattern === "src/auth.ts" && rule.permission === "read-write",
    ),
    false,
  );
  assert.deepEqual(resolved.resolved.findings, [
    {
      ...
      path: "src/auth.ts",
      message:
        'Surface "todos.main" discovered "src/auth.ts", but scope does not allow reading it.',
    },
  ]);
});
...
test("scope deny blocks surface writes even when a broad scope allow matches", () => {
  ...
  assert.equal(
    resolved.resolved.files.some(
      (rule) => rule.pattern === "src/app/secret.ts" && rule.permission === "read-write",
    ),
    false,
  );
  assert.deepEqual(resolved.resolved.findings, [
...
test("shared surface files produce findings instead of write authority", () => {
  ...
  assert.equal(
    resolved.resolved.files.some(
      (rule) => rule.pattern === "src/app/shared/item-card.tsx" && rule.permission === "read-write",
    ),
    false,
  );
  assert.deepEqual(
    resolved.resolved.findings.map((finding) => ({
      code: finding.code,
